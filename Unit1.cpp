...
void sekwencja()
{
    ...
    Form1->Label1->Caption = "Nastepna runda za: 3";
    sndPlaySound ("snd/czas.wav", SND_ASYNC);
    Application->ProcessMessages(); Sleep(1000);
    ...
    for (int i=0; i<ile; i++)
    {
       switch(liczby[i])
       {
          case 1:
          {
          sndPlaySound("snd/d1.wav",SND_SYNC);
...
__fastcall TForm1::TForm1(TComponent* Owner)
        : TForm(Owner)
{
sndPlaySound("snd/tlo.wav", SND_LOOP|SND_ASYNC);
BorderStyle=bsNone;
...
void __fastcall TForm1::z1Click(TObject *Sender)
{
   if(gra_rozpoczeta == true)
   {
      ...
      if(licznik == ile)
      {
         Label1->Caption = "Fantastycznie!";
         sndPlaySound ("snd/wyg.wav", SND_ASYNC);
         sekwencja();
      }
   }
}
